...
#does not apply activation funciton
def _conv_layer(filters, kernel_size, strides=(1, 1), padding='same', name=None):
    return Conv2D(filters, kernel_size, strides=strides, padding=padding,
                  use_bias=True, kernel_initializer='random_normal', name=name) #kernel_initializer='he_normal'
...
class AttentionAugmentation2D(Layer):
    ...
    def build(self, input_shape):
        ...
        self.depth_k, self.depth_v = _normalize_depth_vars(self.depth_k, self.depth_v,
                                                           input_shape)	#this is the wrong call with input_shape, but it doesn't appear to matter b/c they were already normalized before the class obj was created
        #print("new depths, K: ", self.depth_k, " V: ", self.depth_v)
    ...
    def call(self, inputs, **kwargs):
        ...
        qk_shape = [-1, self.num_heads, self._height * self._width, self.depth_k // self.num_heads]
        #v_shape = [self._batch, self.num_heads, self._height * self._width, self.depth_v // self.num_heads]
        v_shape = [-1, self.num_heads, self._height * self._width, self.depth_v // self.num_heads]
        ...
        logits = tf.matmul(flat_q, flat_k, transpose_b=True) #note, this op may run out of space depending on how keras allocates the memory. This will be the limiting factor for how many blocks are able to be augmented 
        #print("dims match")
		#print("\nlogits: ", logits)
    ...
    def split_heads_2d(self, ip):
        static = ip.get_shape().as_list()
        #print("static shape test: ", static)
        tensor_shape = K.shape(ip)
        #print("\ntensor_shape: ", tensor_shape)
        # batch, height, width, channels for axis = -1
        # get the information from static or dynamic 
        ret = []
        for i, static_dim in enumerate(static):
            dim = static_dim or tensor_shape[i]
            ret.append(dim)

        batch, height, width, channels = ret

        #tensor_shape = [tensor_shape[i] for i in range(len(self._shape))]
        #print("tensor_shape after change: ", tensor_shape)
        ...
        #print("\nspatial tensor dimensions, batch: ", batch, " height: ", height, " width: ", width, " channels: ", channels)
        self._batch = batch
        self._height = height
        self._width = width
    ...
    def relative_logits(self, q):
        static = q.get_shape().as_list()
        shape = K.shape(q)
        #print("\nrel q input shape: ", shape)
        #print("rel q input static shape: ", static)
        ...
        #width = shape[3]
        width = ret[3]
        #print("height and width from rel q: ", height, width)

        rel_logits_w = self.relative_logits_1d(q, self.key_relative_w, height, width, transpose_mask=[0, 1, 2, 4, 3, 5])

        rel_logits_h = self.relative_logits_1d(K.permute_dimensions(q, [0, 1, 3, 2, 4]), self.key_relative_h, width, height, transpose_mask=[0, 1, 4, 2, 5, 3])

        return rel_logits_h, rel_logits_w

    def relative_logits_1d(self, q, rel_k, H, W, transpose_mask):
        rel_logits = tf.einsum('bhxyd,md->bhxym', q, rel_k)
        #print("\nrel_logits einsum: ", rel_logits)
        rel_logits = K.reshape(rel_logits, [-1, self.num_heads * H, W, 2 * W - 1])
        #print("\nrel_logits reshape: ", rel_logits)
        rel_logits = self.rel_to_abs(rel_logits)
        #print("\nrel_logits abs: ", rel_logits)
    ...
    def rel_to_abs(self, x):
        ...
        shape = K.shape(x)
        #print("\nshape dynamic rel x: ", shape)
        #print("shape static rel x: ", static)
        ...
        #B, Nh, L, _ = shape
        B, Nh, L, _ = ret
        #print("\nshape data B, Nh, L: ", B, Nh, L)
        
        col_pad = tf.zeros(K.stack([B,Nh,L, 1]))		#K.zeros(K.stack([B,Nh,L,1]))		NOTE: K.zeros does not work in eager mode with tf 2.2!!!
        #print("\ncol_pad: ", col_pad)
        #print("x: ", x)
        x = K.concatenate([x, col_pad], axis=3)
        flat_x = K.reshape(x, [B, Nh, L * 2 * L])
        flat_pad = tf.zeros(K.stack([B, Nh, L - 1]))										#NOTE: K.zeros does not work in eager mode with tf 2.2!!!
...
def augmented_conv2d(ip, filters, kernel_size=(3, 3), strides=(1, 1),
    ...
    #their prev implementation did not properly account for 0 attn or 0 cnn filters
    #this can easily be fixed with a few if statements
    #short circuit the rest of the function if attention is not being used
    if(depth_v == 0):
        conv_out = _conv_layer(filters, kernel_size, strides)(ip)
        output = BatchNormalization()(conv_out)
        return output

    #don't include conv filter if it is fuly attentional
    if(filters == depth_v):
        ...
        output = BatchNormalization()(attn_out)
        return output

    #print("depth_k: ", depth_k, "\tdepth_v:", depth_v)
